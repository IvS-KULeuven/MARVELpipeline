...
fn main() {
    ...
    let flat_image_paths = &data["rawFlatImage"];
    let mask_image_paths = &data["orderMaskImage"];
    ...
    let orders_mask_path = project_root.to_owned() + mask_image_paths.get("outpath").unwrap().as_str().unwrap();
    ...
    println!("Smoothing the master flat image using a Gaussian filter  ");
    ...
    println!("Locating maxima of all orders of all fibers in middle CCD row  ");
