...
fn main() {
    ...
    for (input_path, output_path) in izip!(input_paths, etalon_peak_fitparameters_output_paths) {
        ...
        for ihdu in 1..num_hdus {
            ...
            let xpixel: Vec<i32> = hdu.read_col(&mut input_fits_file, "x pixel fiber 1").unwrap();
            let xpixel: Vec<f64> = xpixel.iter().map(|&value| value as f64).collect();
            let flux:   Vec<f64> = hdu.read_col(&mut input_fits_file, "spectrum fiber 1").unwrap();

            // Read the order information. Example: the key 'EXTNAME' could contain the string "order: 98".

            let order_id: String = hdu.read_key(&mut input_fits_file, "EXTNAME").unwrap();
